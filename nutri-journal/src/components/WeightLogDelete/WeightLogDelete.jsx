...
export default function WeightLogDelete(props) {
  ...
  const deleteWeight = async () => {
    try {
      ...
      const dateExistsResponse = await fetch(
        "/weight/log/delete?" + query.toString(),
        {
          method: "DELETE",
        }
      );
      ...
      if (dateExistsResponse.ok) {
        ...
        if (data.deleted) {
          ...
          console.log(
            "Data with the specified date has been deleted from the database."
          );
        } else {
          swal(
            "Something went wrong! No weight found with the specified date in the database."
          );
          console.log("No data found with the specified date in the database.");
        }
      } else {
        swal(
          "Something went wrong! failed to check if the date exists in the database."
        );
        console.error(
          "Failed to check if the date exists in the database:",
